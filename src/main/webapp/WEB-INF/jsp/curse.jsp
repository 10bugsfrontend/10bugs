			...
			<c:forEach var="cursa" items="${listaCurse}">
				...
				<div class="cont">
					<div class="container-fluid">
						Traseu:
						<c:out value="${cursa.traseu.statieInceput}" />
						-
						<c:out value="${cursa.traseu.statieSosire}" />
						<br>
						<div class="point">
							<c:out value="${Plecare}" />
							<br>
							<fmt:formatDate type="time" pattern="hh:mm"
								value="${cursa.ora_de_plecare}" />
						</div>
						<span>&#8594;</span>
						<div class="point">
							<c:out value="${Sosire}" />
							<br>
							<fmt:formatDate type="time" pattern="hh:mm"
								value="${cursa.ora_de_sosire}" />
						</div>
						<br> Pret:
						<c:out value="${cursa.pret}" />
						<br> Firma:
						<c:out value="${cursa.firma.name}" />
						<br> Capacitate:
						<c:out value="${cursa.capacitate}" />
						<br> Locuri ocupate:
						<c:out value="${cursa.ocupate}" />
						<br> Rating:
						<fmt:formatNumber type="number" maxFractionDigits="1" value="${cursa.firma.rating.scor}" />
						<br> Facilitati:
						<c:out value="${cursa.facilitati}" />
						<br>
						<c:choose>
                    	...
                    	<span class="popuptext" id="myPopup">Cursa a fost rezervata cu succes!</span>
							...
							<c:otherwise>
								<td><a
									href="/secured/listaRezervariPtCursa?id=<c:out value='${cursa.id}' />">Rezervari</a>
								</td>
							</c:otherwise>
						</c:choose>
					</div>
